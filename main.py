...
class getProxy():
    ...
    def test_request(self, num):
        nn_url = "http://www.xicidaili.com/nn/" + str(num)
        r = requests.get(nn_url, headers=self.header, timeout=10)
    ...
    def request_kuai(self, num):
        url = "http://www.kuaidaili.com/free/inha/" + str(num)
        r = requests.get(url, headers=self.header, timeout=10)
        print(r.text)
        et = etree.HTML(r.text)
        trs = et.xpath('//table[@class=\'table table-bordered table-striped\']/tr')
        for tr in trs:
            tds = tr.xpath('//td')
            print "IP:%s\tPort:%s" % (tds[0], tds[1])
            if self.isAlive(tds[0], tds[1]):
                self.insert_db(self.now, tds[0], tds[1])
    ...
    def isAlive(self, ip, port):
        ...
        try:
            r = requests.get(test_url, headers=self.header, timeout=5, proxies=proxy)
